package org.springframework.boot.autoconfigure.mds.mybatis.plus;
...
/**
 * MdsMybatisComponentsBuilder的MybatisPlus实现
 *
 * @author pengpeng
 * @since 2.1
 */
@SuppressWarnings("rawtypes")
public class MybatisPlusMdsMybatisComponentsBuilder extends MdsMybatisComponentsBuilder {
    ...
    public FactoryBean<SqlSessionFactory> buildSqlSessionFactoryBean(String database, DataSource dataSource, MybatisProperties properties) {
        ...
        sqlSessionFactoryBean.setGlobalConfig(getBeanFactory().getBeanProvider(GlobalConfig.class).getIfAvailable()); //MybatisPlus特有的配置
        sqlSessionFactoryBean.setTypeEnumsPackage(getEnvironment().getProperty(MybatisProperties.MYBATIS_PREFIX + ".typeEnumsPackage")); //MybatisPlus特有的配置
    ...
    protected void applyConfiguration(MybatisSqlSessionFactoryBean sqlSessionFactoryBean, String database, MybatisProperties properties) {
        ...
        MybatisConfiguration configuration = (MybatisConfiguration) properties.getConfiguration();
